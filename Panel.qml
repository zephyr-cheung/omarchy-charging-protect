...
Panel {
  ...
  property bool chargeProtectActionRunning: false
  property int chargeThreshold: 0
  property bool chargingProtect: false
  ...
  function setChargingProtect(enabled) {
    ...
    chargeProtectAction.command = [
      "gdbus", "call", "--system",
      "--dest", "org.freedesktop.UPower",
      "--object-path", "/org/freedesktop/UPower/devices/battery_macsmc_battery",
      "--method", "org.freedesktop.UPower.Device.EnableChargeThreshold",
      enabled ? "true" : "false"
    ]
    chargeProtectAction.running = true
  }
  ...
  Process {
    id: chargeProtectProc
    command: ["bash", "-c", "end=$(cat /sys/class/power_supply/macsmc-battery/charge_control_end_threshold 2>/dev/null || true); enabled=$(upower -i /org/freedesktop/UPower/devices/battery_macsmc_battery 2>/dev/null | awk '/charge-threshold-enabled:/ {print $2; exit}'); case $enabled in yes) enabled=1;; *) enabled=0;; esac; printf '%s\t%s\n' $end $enabled"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var parts = String(text || "").trim().split("\t")
        var threshold = Number(parts[0])
        if (isFinite(threshold) && threshold >= 0) root.chargeThreshold = Math.round(threshold)
        root.chargingProtect = parts.length > 1 && parts[1] === "1"
      }
    }
  }

  Process {
    id: chargeProtectAction
    onExited: {
      ...
      Column {
        ...
        Row {
          ...
          Column {
            ...
            InfoPair { label: "Charge limit"; value: root.chargeThreshold > 0 ? root.chargeThreshold + "%" : "—" }
          }
        ...
        Toggle {
          width: parent.width
          label: "Charging protect"
          description: root.chargeThreshold > 0
            ? "Stop charging at " + root.chargeThreshold + "%"
            : "Battery charging threshold unavailable"
          checked: root.chargingProtect
          foreground: root.bar.foreground
          accent: Color.accent
          fontFamily: root.bar.fontFamily
          enabled: root.chargeThreshold > 0 && !root.chargeProtectActionRunning
          onClicked: root.setChargingProtect(!root.chargingProtect)
        }
